...
local Theme = {
    ...
    Outline = Color3.fromRGB(0, 140, 200),
    Button = Color3.fromRGB(35, 35, 50),
    Hover = Color3.fromRGB(50, 50, 70),
    ScanGray = Color3.fromRGB(150, 150, 180),
...
-- Custom Drag Logic for Main Frame (Basitleştirildi)
...
-- Animate open (GÜNCELLENMİŞ - Elastic)
task.delay(0.1, function()
    local goal = { Size = UDim2.new(0, 720, 0, 460), BorderColor3 = Theme.Accent }
...
Header.TextSize = 30 -- Slightly larger
...
-- Close / Minimize button (GÜNCELLENMİŞ)
...
-- Search Button (GÜNCELLENMİŞ Esthetics)
local SearchTabButton = Instance.new("TextButton", Main)
SearchTabButton.Size = UDim2.new(0, 100, 0, 28)
SearchTabButton.Position = UDim2.new(0, 10, 0, 5)
SearchTabButton.Text = "SEARCH"
SearchTabButton.TextColor3 = Theme.Text
SearchTabButton.BackgroundColor3 = Theme.Button
SearchTabButton.Font = Enum.Font.GothamBold
SearchTabButton.TextSize = 14
...
SearchTabButton.MouseLeave:Connect(function() tweenColor(SearchTabButton, Theme.Button) end)


-- 🌟 Auto Scan Toggle Button (GÜNCELLENMİŞ)
local AutoScanButton = Instance.new("TextButton", Main)
AutoScanButton.Size = UDim2.new(0, 100, 0, 28)
AutoScanButton.Position = UDim2.new(0, 115, 0, 5)
...
autoScanStroke.Thickness = 1
autoScanStroke.Transparency = 0.5

-- Buton rengini ve metnini güncelleyen yardımcı fonksiyon (Tweenli)
...
-- Left Panel — Leaked users (BOŞLUK DÜZELTİLDİ VE DISCORD EKLENDİ)
local HEADER_BUTTON_HEIGHT = SearchTabButton.Size.Y.Offset -- 28
local HEADER_PADDING_Y = SearchTabButton.Position.Y.Offset -- 5
local LEFT_PANEL_Y_START = HEADER_BUTTON_HEIGHT + (HEADER_PADDING_Y * 2) -- 28 + 10 = 38 (Boşluk yok)
local LeftPanelOffset = 0 -- Header Yüksekliği
local LeftPanel = Instance.new("Frame", Main)
-- Main Y - Header Y (40) - Left Panel'in üstten başlayan Y'si
LeftPanel.Size = UDim2.new(0, 220, 1, -(LeftPanelOffset + LEFT_PANEL_Y_START - HEADER_PADDING_Y + 20)) -- 20 discord yazısı için ek boşluk
LeftPanel.Position = UDim2.new(0, 10, 0, LEFT_PANEL_Y_START) -- 38 Y'den başlar
...
ScanProgressLabel.TextColor3 = Theme.Text
ScanProgressLabel.Font = Enum.Font.Gotham
ScanProgressLabel.TextSize = 13
ScanProgressLabel.Text = "Checked: 0/0 x0"
...
LeftScroll.Size = UDim2.new(1, 0, 1, -46 - 20) -- Discord yazısı için alttan boşluk
...
DiscordLabel.Position = UDim2.new(0, 5, 1, -20) -- En alta hizalı
...
-- Right panel / Terminal (GÜNCELLENMİŞ)
...
-- Info Button (GÜNCELLENMİŞ Esthetics)
...
--// LOGGING
local function terminalLog(msg, logType)
...
local function openUserInfoTab(username)
    ...
    -- 💡 YALNIZCA SAĞDA KOPYALA BUTONU OLAN makeInfoLineWithCopy
...
-- Info tab (GÜNCELLENMİŞ VE GÜZELLEŞTİRİLMİŞ)
local function openInfoTab()
    ...
    layout.Padding = UDim.new(0, 8) -- Daha fazla boşluk
    ...
    local function makeTitle(text)
        ...
        lbl.BackgroundTransparency = 1
        lbl.Font = Enum.Font.SciFi -- Daha dikkat çekici başlık fontu
        lbl.TextSize = 18
        lbl.TextXAlignment = Enum.TextXAlignment.Left
        Instance.new("UIListLayout", lbl):Destroy() -- list item olarak eklendiği için layout bozulmasın
    end
    ...
    makeLine("Enjoy this poorly coded AI slop script for free. Btw this shit costs $45 so enjoy 😘", Theme.Green)
    
    makeTitle("SCANNING MODULE")
    makeLine("• Live User Scanning provides real-time leak checks for players joining the server.", Theme.Text)
    makeLine("• Auto Scan (SCAN: ON) enables continuous checking and rescan attempts.", Theme.Text)
    makeLine("• Results may not be 100% accurate, but data with 'roblox' origin is verified and guaranteed real.", Color3.fromRGB(255, 200, 0))
    ...
    makeLine("© 2024 Kryptonite Team (lol no). Leaked by Stalkie.", Theme.ScanGray)
end
...
local function openSearchPanel()
    ...
    makeStaticLogLine("[INFO] Enter search term below. Use only one field.", Theme.Green)
    ...
    EmailBox.PlaceholderText = "Email or Username..."
    ...
    PasswordBox.PlaceholderText = "Enter password to search..."
    ...
    RunButtonFrame.Size = UDim2.new(1, 0, 0, 26) 
    ...
    SearchRunButton.Size = UDim2.new(0, 100, 1, 0)
    SearchRunButton.Position = UDim2.new(0.5, -50, 0, 0)
    SearchRunButton.Text = "RUN SEARCH"
    SearchRunButton.TextColor3 = Theme.Text
    SearchRunButton.BackgroundColor3 = Theme.Accent
    SearchRunButton.Font = Enum.Font.GothamBold
    SearchRunButton.TextSize = 14
    Instance.new("UICorner", SearchRunButton).CornerRadius = UDim.new(0, 5)
    ...
    SearchRunButton.MouseLeave:Connect(function() tweenColor(SearchRunButton, Theme.Accent) end)

    makeStaticLogLine("──────────────────────────────────────────────", Theme.ScanGray)
    makeStaticLogLine("[LEAKED] RESULTS:", Theme.Red)
    
    local SEARCH_HEADER_FIXED_HEIGHT = 164 
    ...
    -- 💡 SEARCH TAB İÇİN KOPYALAMA FONKSİYONU
    ...
    local function displaySearchResults(results, searchTerm)
        ...
        if not results or results.found == 0 or not results.result then
            makeDynamicResultLine(string.format("[INFO] No records found for query: %s", searchTerm), Theme.Green)
        else
            makeDynamicResultLine(string.format("[WARN] Total %d records found.", results.found), Color3.fromRGB(255, 200, 0))
            makeDynamicResultLine("----------------------------------------------", Theme.ScanGray)
            ...
            for i, entry in ipairs(results.result) do
                ...
                titleLbl.Text = string.format("[RECORD #%d]", i)
                ...
                makeSearchDataLineWithCopy(recordFrame, "  User: ", entry.username or "N/A", Theme.Text)
                makeSearchDataLineWithCopy(recordFrame, "  Pass: ", entry.password or "N/A", Theme.Red)
                makeSearchDataLineWithCopy(recordFrame, "  Email: ", entry.email or "N/A", Theme.Green)
                
                local originStr = table.concat(entry.origin or {}, ", ")
                if originStr == "" and entry.source and entry.source.name then
                    originStr = entry.source.name
                end
                makeSearchDataLineWithCopy(recordFrame, "  Source: ", originStr or "N/A", Theme.Accent)
    ...
    local function runSearchAPI()
        ...
        searchRunning = true
        SearchRunButton.Text = "LOADING..."

        terminalLog("External search initiated: " .. searchTerm, "scan")
...
local function addLeakedUser(entry)
    ...
    if not usersWithLogs[lower] then
        ...
        btn.Text = "⚠ " .. (entry.username or "N/A")
        ...
        terminalLog("User leaked: " .. lower, "leak")
    end
...
local function scanUser(username)
    terminalLog("Scanning: " .. username, "scan")
    local requestFn = syn and syn.request or http and http.request or request
    if not requestFn then
        terminalLog("HTTP unavailable", "error")
        return
    ...
    if not leakcheck or leakcheck.found <= 0 then
        terminalLog(username .. " is clean.", "info")
        return
    end
    ...
    if foundLeaks then
        ...
        terminalLog(username .. " is clean (no usable records found).", "info")
    end
end
-- Process queue
    ...
    task.spawn(function()
        ...
        while #scanQueue > 0 do
            ...
            if not scannedPlayers[lower] then
                ...
                ScanProgressLabel.Text = string.format("Checked: %d/%d x%d", scannedThisRound, totalThisRound, scanRounds)
...
-- Queue user
local function queueUser(username, instance)
    if not isAutoScanEnabled then return end 
...
-- Minimize / restore logic (GÜNCELLENMİŞ - Smooth Tween)
